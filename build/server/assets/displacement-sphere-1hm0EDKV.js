...
import { g as useTheme, u as useInViewport, b as useWindowSize, e as media, T as Transition } from "./server-build-L_i-112A.js";
...
const DisplacementSphere = (props) => {
  ...
  useEffect(() => {
    ...
    material.current.onBeforeCompile = (shader) => {
      uniforms.current = UniformsUtils.merge([
        shader.uniforms,
        { time: { type: "f", value: 0 } },
        { themeMode: { value: theme === "light" ? 1 : 0 } }
      ]);
  ...
  useEffect(() => {
    var _a;
    const dirLight = new DirectionalLight(16726843, theme === "light" ? 1.1 : 1.8);
    const ambientLight = new AmbientLight(2752512, theme === "light" ? 0.9 : 0.45);
    if ((_a = uniforms.current) == null ? void 0 : _a.themeMode) {
      uniforms.current.themeMode.value = theme === "light" ? 1 : 0;
    }
  ...
  useEffect(() => {
    const onMouseMove = throttle((event) => {
      ...
      sphere.current.rotation.z += 18e-4;
    }, 100);
